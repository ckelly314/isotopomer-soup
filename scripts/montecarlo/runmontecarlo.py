...
def runmontecarlo(station, feature, iters, weights=None):
    ...
    def simulation(i):
        ...
        Inputs:
        i: number for this simulation
        iters: number of model iterations
        montecarloNH4, montecarloNO2, montecarloNO3: Numpy arrays for each tracer experiment,
        generated by genmontecarlo.py
        bgcNH4, bgcNO2, bgcNO3: BioGeoChemistry objects containing biogeochemical parameters
        for each tracer experiment, generated by bgc.py
        trNH4, trNO2, trNO3: Tracer objects containing initialized arrays of state variables,
        generated by  tracers.py
        isos: isotope effects
        params: model params
        weights: numpy array containing weights for each of the three tracer experiments
        (can be tuned for a better fit)

        Outputs:
        output: save simulation results to "montecarlo.csv"
        """
        ...
        result = minimize(
            modelv5objective, x, args=args, method="nelder-mead", bounds=bnds, options={'maxfev' : 500, 'fatol': 0.1}
