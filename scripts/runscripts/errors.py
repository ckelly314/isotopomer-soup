...
def run_errors(station, feature):
    ...
    def objective(x): #, bgc, isos, tracers, modelparams):
        ...
        tracersNH4 = modelv4(x, bgcNH4, isos, trNH4, params)
        ...
        tracersNO2 = modelv4(x, bgcNO2, isos, trNO2, params)
        ...
        tracersNO3 = modelv4(x, bgcNO3, isos, trNO3, params)
    ...
    #x = [kestimateNH4, kestimateNO2, kestimateNO3, kestimate_hybrid1, kestimate_hybrid2] # for modelv4
    ...
    for i in range(3):
        ...
        result = minimize(objective, pt, method='nelder-mead', bounds=bnds, options={'maxfev' : 500, 'fatol': 0.1})
        ...
        tracersNH4 = modelv4(result.x, bgcNH4, isos, trNH4, params)
        tracersNO2 = modelv4(result.x, bgcNO2, isos, trNO2, params)
        tracersNO3 = modelv4(result.x, bgcNO3, isos, trNO3, params)

        outputNH4 = postprocess(bgcNH4, isos, tracersNH4, result.x, model="modelv4")
        outputNO2 = postprocess(bgcNO2, isos, tracersNO2, result.x, model="modelv4")
        outputNO3 = postprocess(bgcNO3, isos, tracersNO3, result.x, model="modelv4")
        ...
        modeloutput = pd.read_excel("scripts/data/00_modelv4error.xlsx",
                                   index_col="Key")
        modeloutput = modeloutput.append(saveout)
        modeloutput.to_excel("scripts/data/00_modelv4error.xlsx")
        ...
        if i==2:
            ...
            scatter_plot(data=inputdata, station=stn, feature=ft, tracer="NH4+",
                         modeloutput=outputNH4, filename=f"Figures/errors/{stn}{ft}NH4+modelv4.pdf")

            scatter_plot(data=inputdata, station=stn, feature=ft, tracer="NO2-",
                         modeloutput=outputNO2, filename=f"Figures/errors/{stn}{ft}NO2-modelv4.pdf")

            scatter_plot(data=inputdata, station=stn, feature=ft, tracer="NO3-",
                         modeloutput=outputNO3, filename=f"Figures/errors/{stn}{ft}NO3-modelv4.pdf")
