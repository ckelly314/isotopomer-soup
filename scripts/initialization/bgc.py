...
class BioGeoChemistry:
	...
	def __init__(self, key, tracer):
		...
		self.station = data.station # "PS1" = offshore, "PS2" = center of ODZ, "PS3" = coastal
		self.feature = data.feature # descriptive name of the target water column feature
		...
		if tracer=="NH4+":
			...
			self.AFNO2_init = (self.afNO2_ambient*self.NO2_ambient + self.af_carrier*self.NO2_carrier)/self.NO2_init
			_, self.d15NO2_init = convert_af(self.AFNO2_init)

			self.NH4_init = self.NH4_ambient + self.NH4_spike # nmol/L
			self.AFNH4_init = (self.afNH4_ambient*self.NH4_ambient + self.af_spike*self.NH4_spike)/self.NH4_init
			...
			self.AFNO3_init = (self.afNO3_ambient*self.NO3_ambient + self.af_spike*self.NO3_spike)/self.NO3_init
			_, self.d15NO3_init = convert_af(self.AFNO3_init)
		...
		try:
			self.kNH4TONO2 = data.kNH4TONO2 / (self.NH4_ambient + self.NH4_spike) # convert rate in nM/day to 1st-order rate constant
