...
def postprocess(bgc, isos, tracers, x, model):
    ...
    nh4_concentration = pd.DataFrame(
        tracers.nh4_14 + tracers.nh4_15, columns={"[NH4+]_nM"}
    )
    nh4_14 = pd.DataFrame(tracers.nh4_14, columns={"[14NH4+]_nM"})
    nh4_15 = pd.DataFrame(tracers.nh4_15, columns={"[15NH4+]_nM"})
    d15nh4 = pd.DataFrame(
        (((tracers.nh4_15 / (tracers.nh4_14)) / R15std) - 1) * 1000, columns={"d15NH4+"}
    )
    afnh4_df = pd.DataFrame(tracers.afnh4, columns={"AFNH4+"})

    nh2oh_concentration = pd.DataFrame(
        tracers.nh2oh_15 + tracers.nh2oh_14, columns={"[NH2OH]_nM"}
    )
    ...
    no2_concentration = pd.DataFrame(
        tracers.no2_14 + tracers.no2_15, columns={"[NO2-]_nM"}
    )
    ...
    d15no2 = pd.DataFrame(
        (((tracers.no2_15 / (tracers.no2_14)) / R15std) - 1) * 1000, columns={"d15NO2-"}
    )
    afno2_df = pd.DataFrame(tracers.afno2, columns={"AFNO2-"})

    no3_concentration = pd.DataFrame(
        tracers.no3_14 + tracers.no3_15, columns={"[NO3-]_nM"}
    )
    no3_14 = pd.DataFrame(tracers.no3_14, columns={"[14NO3-]_nM"})
    no3_15 = pd.DataFrame(tracers.no3_15, columns={"[15NO3-]_nM"})
    d15no3 = pd.DataFrame(
        (((tracers.no3_15 / (tracers.no3_14)) / R15std) - 1) * 1000, columns={"d15NO3-"}
    )
    afno3_df = pd.DataFrame(tracers.afno3, columns={"AFNO3-"})

    n2o_concentration = pd.DataFrame(
        tracers.n2o_44 + tracers.n2o_45a + tracers.n2o_45b + tracers.n2o_46,
        columns={"[N2O]_nM"},
    )
    ...
    d15Na = pd.DataFrame(
        (((tracers.n2o_45a / (tracers.n2o_44)) / R15std) - 1) * 1000,
        columns={"d15N2Oa"},
    )
    d15Nb = pd.DataFrame(
        (((tracers.n2o_45b / (tracers.n2o_44)) / R15std) - 1) * 1000,
        columns={"d15N2Ob"},
    )
    d18O = pd.DataFrame(
        (((tracers.n2o_46 / (tracers.n2o_44)) / R15std) - 1) * 1000, columns={"d18O"}
    )

    n2_concentration = pd.DataFrame(
        tracers.n2_28 + tracers.n2_29 + tracers.n2_30, columns={"[N2]_nM"}
    )
    ...
    output["NO2TONO3"] = (
        bgc.kNO2TONO3 * tracers.no2_14
        + bgc.kNO2TONO3 / isos.alpha15NO2TONO3 * tracers.no2_15
    )
    output["NO3TONO2"] = (
        bgc.kNO3TONO2 * tracers.no3_14
        + bgc.kNO3TONO2 / isos.alpha15NO3TONO2 * tracers.no3_15
    )
    ...
    if model == "modelv1":
        ...
        output["NH4TONO2"] = (
            bgc.kNH4TONO2 * tracers.nh4_14
            + bgc.kNH4TONO2 / isos.alpha15NH4TONO2AOA * tracers.nh4_15
        )

        [knitrification, kdenitno2, kdenitno3, khybrid1, khybrid2] = x

        output["nitrification"] = knitrification * (
            (tracers.nh4_14 + tracers.nh4_15) ** 2
        )
        output["denitno2"] = kdenitno2 * ((tracers.no2_14 + tracers.no2_15) ** 2)
        output["denitno3"] = kdenitno3 * ((tracers.no3_14 + tracers.no3_15) ** 2)
        output["hybrid1"] = (
            khybrid1
            * (tracers.nh4_14 + tracers.nh4_15)
        ...
        output["NH4TONO2AOB"] = (
            kAOB * tracers.nh4_14 + kAOB * isos.alpha15NH4TONO2AOB * tracers.nh4_15
        )
        output["NH4TONH2OH"] = (
            kAOA * tracers.nh4_14 + kAOA / isos.alpha15NH4TONO2AOA * tracers.nh4_15
        )
        output["NH2OHTONO"] = (
            kAOA * tracers.nh2oh_14 + kAOA / isos.alpha15NH4TONO2AOA * tracers.nh2oh_15
        )
        output["NOTONO2"] = (
            kAOA * tracers.no_14 + kAOA / isos.alpha15NH4TONO2AOA * tracers.no_15
        )
        output["NO2TONO"] = (
            bgc.kNO2TONO * tracers.no2_14
            + bgc.kNO2TONO / isos.alpha15NO2TONO * tracers.no2_15
        )
        ...
        output["hybrid1"] = (
            khybrid1
            * (tracers.nh2oh_14 + tracers.nh2oh_15)
            * (tracers.no_14 + tracers.no_15)
        )
        output["hybrid2"] = (
            khybrid2
            * (tracers.nh2oh_14 + tracers.nh2oh_15)
    ...
    output["check_mass_conservation"] = output.loc[
        :,
        [
            "[NH4+]_nM",
            "[NH2OH]_nM",
